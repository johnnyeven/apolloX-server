...
--
-- 表的结构 `server_list_view`
--

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `server_list_view` AS select `pulse_db_game`.`game_product`.`game_id` AS `game_id`,`pulse_db_game`.`game_product`.`game_name` AS `game_name`,`pulse_db_game`.`game_product`.`game_version` AS `game_version`,`pulse_db_game`.`game_product`.`auth_key` AS `auth_key`,`pulse_db_game`.`section_list`.`section_name` AS `section_name`,`pulse_db_game`.`server_list`.`account_server_section` AS `account_server_section`,`pulse_db_game`.`server_list`.`account_server_id` AS `account_server_id`,`pulse_db_game`.`server_list`.`server_name` AS `server_name`,`pulse_db_game`.`server_list`.`server_ip` AS `server_ip`,`pulse_db_game`.`server_list`.`server_port` AS `server_port`,`pulse_db_game`.`server_list`.`server_message_port` AS `server_message_port`,`pulse_db_game`.`server_list`.`server_max_player` AS `server_max_player`,`pulse_db_game`.`server_list`.`account_count` AS `account_count`,`pulse_db_game`.`server_list`.`server_language` AS `server_language`,`pulse_db_game`.`server_list`.`server_recommend` AS `server_recommend` from ((`game_product` join `section_list` on((`pulse_db_game`.`section_list`.`game_id` = `pulse_db_game`.`game_product`.`game_id`))) join `server_list` on((`pulse_db_game`.`server_list`.`account_server_section` = `pulse_db_game`.`section_list`.`server_section_id`)));
-- 读取数据时发生错误: (#1356 - View 'pulse_db_game.server_list_view' references invalid table(s) or column(s) or function(s) or definer/invoker of view lack rights to use them)
--
